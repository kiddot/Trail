            ...
            public void run()
            {
                ...
                if (!QoS4ReceiveDaemonC2S.this._excuting)
                {
                    ...
                    if (QoS4ReceiveDaemonC2S.DEBUG) {
                        QoS4ReceiveDaemonC2S.logger.debug("【IMCORE】【QoS接收方】++++++++++ START 暂存处理线程正在运行中，当前长度" + QoS4ReceiveDaemonC2S.this.recievedMessages.size() + ".");
                    }

                    for (String key : QoS4ReceiveDaemonC2S.this.recievedMessages.keySet())
                    {
                        long delta = System.currentTimeMillis() - ((Long)QoS4ReceiveDaemonC2S.this.recievedMessages.get(key)).longValue();
                        ...
                        QoS4ReceiveDaemonC2S.this.recievedMessages.remove(key);
                    }
                }

                if (QoS4ReceiveDaemonC2S.DEBUG) {
                    QoS4ReceiveDaemonC2S.logger.debug("【IMCORE】【QoS接收方】++++++++++ END 暂存处理线程正在运行中，当前长度" + QoS4ReceiveDaemonC2S.this.recievedMessages.size() + ".");
                }
